...
function AboutH2({ text }: AboutH2Type) {
  return (
    <h2 className="mb-4 font-serif text-2xl font-normal text-rose-100 text-shadow-md/15">
      {text}
    </h2>
  );
}
...
function GlowingBackground({ children, glowClass }: GlowingBackgroundType) {
  ...
  const filter = useTransform(brightness, (b) => `blur(24px) brightness(${b})`);

  return (
    <div ref={ref} className="relative flex w-full flex-col items-center">
      <motion.div
        style={{ filter }}
        className={`absolute inset-[-5%] -z-10 scale-90 transform-gpu rounded-4xl will-change-transform ${glowClass} bg-clip-content blur-md`}
      />
      {children}
    </div>
  );
...
export default function About() {
  return (
    <main className="mx-auto flex h-fit w-full max-w-120 flex-col gap-4 p-2 pt-0">
      <H1 text={"About Me."} />
      ...
      <div className="mb-8 flex flex-col gap-8 px-4">
        <GlowingBackground glowClass="bg-radial from-fuchsia-800/50 from-40% to-fuchsia-900/40">
          <div className="m-8 w-full px-6 text-center">
            ...
            <p className="mx-4 font-mono text-base/6 whitespace-pre-line text-orange-100/95 text-shadow-sm">
              <UseAnimatedText
